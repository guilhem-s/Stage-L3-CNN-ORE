...
def create_data(IM_DIR, ethnie, proportion): 
    ...
    for imname in os.listdir(IM_DIR):
        data.append([np.array(cv2.imread(imname, cv2.IMREAD_GRAYSCALE)), np.random.randint(0,2, cpt_final), imname])
...
def create_model(nb_cc_value, nb_im):
    ...
    model.add(Conv2D(16, (5, 5), activation=lrelu, input_shape=(300, 300, 1)))
    ...
    model.add(Dense(120, activation=lrelu))
...
def processing_data(data, nb_im):
    ...
    for i in range(0,len(data)):
            ...
            lab_train.append(data[i][1]) # label de l'image
    ...
    return xtrain, ytrain, name_train, lab_train, datagen.flow(xtrain, ytrain, batch_size=32)
...
os.chdir(dirname)
#Processing data
xtrain, ytrain, name_train, lab_train, train_iterator = processing_data(data, nb_im)
